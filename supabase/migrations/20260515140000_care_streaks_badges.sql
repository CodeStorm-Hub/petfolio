CREATE TABLE IF NOT EXISTS public.care_streaks (
  pet_id               uuid NOT NULL PRIMARY KEY REFERENCES public.pets (id) ON DELETE CASCADE,
...
CREATE POLICY "care_streaks: select by pet owner"
  ON public.care_streaks FOR SELECT TO authenticated
  USING (
    (SELECT auth.uid()) IN (
      SELECT p.owner_id FROM public.pets p WHERE p.id = care_streaks.pet_id
    )
  );

CREATE POLICY "pet_badges: select by pet owner"
  ON public.pet_badges FOR SELECT TO authenticated
  USING (
    (SELECT auth.uid()) IN (
      SELECT p.owner_id FROM public.pets p WHERE p.id = pet_badges.pet_id
    )
  );

CREATE OR REPLACE FUNCTION public.check_daily_completion (target_pet_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
REVOKE ALL ON FUNCTION public.check_daily_completion (uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.check_daily_completion (uuid) TO authenticated;

GRANT SELECT ON TABLE public.care_streaks TO authenticated;
GRANT SELECT ON TABLE public.pet_badges TO authenticated;
