...
- Daily streak completion is driven by the `check_daily_completion` RPC against `care_tasks` and `care_logs`, with streak state in `care_streaks` and badges in `pet_badges`.
